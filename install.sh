...
OS_VERSION=""
OS_NUM=""
FREERADIUS_VERSION="3.2.5"
CURRENT_DIR=""
PLUGIN_NAME=""
TOTAL_ARGS=$*
result=""
AINSTALL=1
export AINSTALL #tell external scripts that AInstall is running
...
show_dialog () {
 ...
 while read line; do
   ...
   if echo ${NAME} | grep -q "update\|mysql\|apache\|perl_modules\|freeradius\|utils"; then
     vars=${vars}"${NAME} \"${DESCRIBE}\" on ";
   else
     vars=${vars}"${NAME} \"${DESCRIBE}\" off ";
   fi
...
install_freeradius() {
 ...
 _install make gmake gcc libmysqlclient-dev libmysqlclient16 libmysqlclient18 libperl-dev
...
fetch_free_distro () {
  ...
  if [ -d /usr/abills ]; then
     echo "ABillS exists at /usr/abills/. Will not download"
     return;
  fi

  if [ "${INSTALL_VERSION}" != "" ]; then
    ABILLS_VERSION=`echo "${INSTALL_VERSION}" | awk -F. '{print $1"."$2 }'`;
    VERSION_PREFIX=`echo "${INSTALL_VERSION}" | awk -F. '{print "."$3 }'`;
...
for _switch ; do
        ...
        -t)     start_tmux_session
                ...
                shift; shift
                ;;
        --install-version) INSTALL_VERSION=$2
                shift; shift;
                ;;
...
cd ${BILLING_DIR}/misc/
mkdir -p /usr/abills/var/ /usr/abills/var/log/ /usr/abills/var/log/ipn/
touch /usr/abills/var/log/sql_errors
chmod -R +w /usr/abills/var
